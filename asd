...
BEGIN
    ...
    -------------------------------------------------------------------
    -- Por defecto: usar @DiasAtras días hacia atrás desde hoy
    SET @FechaDesde = DATEADD(DAY, -@DiasAtras, CAST(GETDATE() AS DATE));

    -- Si querés probar meses en lugar de días, podés cambiar la línea anterior por:
    -- 1 mes atrás:
    ...
    -------------------------------------------------------------------
    -- CTE DE EVENTOS
    -- - TieneEntregaReciente: ENTREGADA (EVTTDI = 1) dentro de @FechaDesde
    -- - TieneEstadoFinal: cualquier evento 16/19/70 en su historial
    -------------------------------------------------------------------
    ;WITH EVT AS (
        SELECT
              E.DOCID
